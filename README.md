...
`tui --run <run-id>` открывает dependency-free terminal UI для live monitor and checkpoint review. `status --tui` starts on Overview, `review --tui` starts on Review. The default `simple` view leads with the operator next action, a filtered Timeline, checkpoint/version summary, and the Review inbox. Press `v` to cycle `simple -> verbose -> extended`; `extended` keeps the raw event stream for forensic debugging. `--view simple|verbose|extended` selects the initial detail level. Keys: `1-4` panels, `j/k` or arrows move, `Enter` accepts the suggested review action, `r/e/m/a` set the selected review action, `A` approves repair for all review items, `x` expands selected details, `n` edits a note, `s` saves decisions, `q` exits.

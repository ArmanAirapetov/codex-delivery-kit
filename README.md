...
Если run остановился на `Human review is required`, используйте `review --run <run-id>` для guided terminal review. Команда собирает inbox из failed validation commands, непроверенных acceptance criteria и blocking review findings, показывает action descriptions и suggested action для каждого пункта, сохраняет decisions только внутри `.codex/delivery-runs/<run-id>/` и печатает безопасную команду resume. Interactive output использует цвета в TTY; `--no-color` или `NO_COLOR=1` отключают ANSI. `review --json` работает read-only на любой фазе и удобен для внешнего UI.

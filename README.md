...
- Git;
- Node.js 22+;
- npm for the bundled Ink/React TUI runtime;
...
Если run остановился на `Human review is required`, используйте `review --run <run-id>` для guided terminal review или `review --tui --run <run-id>` для Cockpit/Review/Timeline/Workspace/Diagnostics TUI. Оба режима собирают inbox из failed validation commands, непроверенных acceptance criteria и blocking review findings, показывают action descriptions и suggested action для каждого пункта, сохраняют decisions только внутри `.codex/delivery-runs/<run-id>/` и печатают безопасную команду resume. Missing project dependencies such as `pytest`, `tsc`, Vite or Vitest default to `repair_requested`, meaning the operator approves Codex to fix requirements/package manifests and lets resume prepare checked-in Python/npm dependencies before validation. Missing system tools, services or credentials default to `environment_required`. Interactive output использует цвета в TTY; `--no-color` или `NO_COLOR=1` отключают ANSI. `review --json` работает read-only на любой фазе и удобен для внешнего UI.

`tui --run <run-id>` открывает Ink/React terminal UI для live monitor and checkpoint review. The installer runs `npm ci --omit=dev --prefix .codex/delivery-kit` to install the nested TUI runtime. `status --tui` starts on the Autopilot Cockpit; `review --tui` starts on Review. `tui --once --no-color` renders one frame without an interactive terminal for scripts, snapshots, and smoke tests. The cockpit leads with run progress, elapsed/phase time, conservative ETA, active work, Timeline snapshot, and an Intervention Queue that surfaces only operator-required actions. Durable actions are two-step: `s` proposes saving review decisions and `R` proposes background resume, then `y` confirms or `Esc`/`n` cancels. Press `v` to cycle `normal -> detail -> raw` (`simple|verbose|extended` remain accepted); raw keeps the event stream for forensic debugging. Panels: `1` Cockpit, `2` Review, `3` Timeline, `4` Workspace, `5` Diagnostics. Legacy `--panel overview|events|checkpoints` aliases still work. Review keys remain: `Enter` accepts the suggested review action, `r/e/m/a` set the selected action, `A` approves repair for all items, `x` expands details, `n` edits a note. `!` explicitly allows/disables dirty resume for the current TUI session, `?` toggles help, and `q` exits. `--no-time` hides elapsed-time prefixes in human progress/log/TUI output; JSON status still includes timing fields.

`web` starts a local Vite/React operator console over the same run artifacts and actions. It binds to `127.0.0.1` by default, prints an ephemeral tokenized URL, serves Cockpit/Timeline/Review/Workspace/Diagnostics views, streams live updates with SSE, and supports confirmation-gated review save, background resume, stop, and integration cleanup. The installer builds the web assets after `npm ci`; for development use `npm --prefix .codex/delivery-kit run web:dev`.

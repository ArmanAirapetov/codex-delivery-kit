...
async function main() {
  ...
  try {
    ...
    await readFile(path.join(temp, '.codex', 'delivery-kit', 'web-server.mjs'), 'utf8');
    await readFile(path.join(temp, '.codex', 'delivery-kit', 'web', 'src', 'App.tsx'), 'utf8');
    ...
    await readFile(path.join(temp, 'scripts', 'web-smoke.mjs'), 'utf8');
